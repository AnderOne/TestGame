...
namespace TestGame {
	...
	public class MainField: DrawableGameComponent {
		//...
		private class Item {
			//...
		...
		//...
		private const double TIME_FOR_UPDATE = 0.5;
		private const int STEP_FOR_UPDATE = 10;
		//...
		...
		private int score = 0;
		//...
		public MainField(Game gm, Rectangle rec, Point num, double timeLimit): base(gm) {
			//...
		...
		public override void Draw(GameTime gameTime) {
			...
			//...
			int s = Convert.ToInt32(STEP_FOR_UPDATE * (time / TIME_FOR_UPDATE));
			//...
			for (int ix = 0; ix < num.X; ++ ix) for (int iy = 0; iy < num.Y; ++ iy) {
				var it = items[ix, iy];
				...
				if (it.stat == Item.Stat.IS_MOVE) {
					r = new Rectangle(r.X + (it.off.X * len.X * s) / STEP_FOR_UPDATE,
					                  r.Y + (it.off.Y * len.Y * s) / STEP_FOR_UPDATE, r.Width, r.Height);
				}
				if (it.stat == Item.Stat.IS_INIT) {
					int sx = (len.X * Math.Max(0, (STEP_FOR_UPDATE - s))) / STEP_FOR_UPDATE;
					int sy = (len.Y * Math.Max(0, (STEP_FOR_UPDATE - s))) / STEP_FOR_UPDATE;
					r = new Rectangle(r.X + sx / 2, r.Y + sy / 2, r.Width - sx, r.Height - sy);
				}
				if (it.stat == Item.Stat.IS_FREE) {
					int sx = (len.X * Math.Min(STEP_FOR_UPDATE, s)) / STEP_FOR_UPDATE;
					int sy = (len.Y * Math.Min(STEP_FOR_UPDATE, s)) / STEP_FOR_UPDATE;
					r = new Rectangle(r.X + sx / 2, r.Y + sy / 2, r.Width - sx, r.Height - sy);
				}
			...
			if ((stat == Stat.DO_WAIT) && (curr != null)) {
				...
				if (s >= STEP_FOR_UPDATE / 2) s = STEP_FOR_UPDATE - s;
				int sx = (len.X * s) / STEP_FOR_UPDATE;
				int sy = (len.Y * s) / STEP_FOR_UPDATE;
				r = new Rectangle(r.X - sx / 2, r.Y - sy / 2, r.Width + sx, r.Height + sy);
		...
		public override void Update(GameTime gameTime) {
			...
			//...
			if (time < TIME_FOR_UPDATE) return;
			//...
			switch (stat) {
